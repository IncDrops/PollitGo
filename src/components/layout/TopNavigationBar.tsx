...
const GreenCheckDot = () => (
  <svg 
    width="1em" 
    height="1em" 
    viewBox="0 0 28 28" // Increased viewBox for more space for the circle relative to check
    fill="none" 
    xmlns="http://www.w3.org/2000/svg" 
    style={{ 
      display: 'inline-block', 
      verticalAlign: 'middle', // Adjusted for better alignment
      transform: 'translateY(-0.15em)', // Fine-tune vertical position
      marginLeft: '-0.1em', // Adjust to overlap original dot position
      marginRight: '0.05em' // Space before next char
    }}
    className="h-[0.6em] w-[0.6em]" // Control size relative to font
  >
...
export default function TopNavigationBar() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 h-[70px] bg-nav-background border-b border-border shadow-sm z-50">
      <div className="container mx-auto h-full flex items-center justify-between px-4">
        <Link href="/" passHref>
          <div className="flex flex-col items-start cursor-pointer">
            ...
            <p className="text-xs font-alice text-black -mt-0.5" style={{letterSpacing: '0.05em'}}>
              THE 2nd OPINION APP
            </p>
          </div>
        </Link>
