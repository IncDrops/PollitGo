...
const usersData: { name: string }[] = [
  ...
  { name: 'Ugly Day Canceller' }, { name: 'Spaghetti Fiend' }, { name: 'DJ Drama Llama' }
];
...
type PollSkeleton = Omit<Poll, 'totalVotes' | 'isVoted' | 'votedOptionId' | 'commentsCount' | 'likes' | 'tipCount' | 'pledgeAmount' | 'pledgeOutcome' | 'imageKeywords' | 'isSpicy'> & { isSpicy?: boolean };
...
const twoOptionPolls: PollSkeleton[] = [
  ...
  { id: '2opt_toxic_closure_ghosted', creator: getRandomUser(), question: "Toxic trait: need closure from someone I ghosted. Message them?", imageUrls: [`https://placehold.co/600x400.png?text=${urlSafeText("Ghost Closure")}`], options: [{ id: '2opt_toxic_a', text: "Yes, give whiplash" }, { id: '2opt_toxic_b', text: "No, vanish like smoke" }], deadline: new Date(Date.now() + parseTimeRemaining("3 hours")).toISOString(), createdAt: generateCreatedAt("3 hours"), isSpicy: true },
  ...
  { id: '2opt_situationship_risk', creator: getRandomUser(), question: "Situationship too comfy. Risk it by asking 'What are we?'", imageUrls: [`https://placehold.co/600x400.png?text=${urlSafeText("Situationship")}`], options: [{ id: '2opt_situation_a', text: "Yes, clarity or chaos!" }, { id: '2opt_situation_b', text: "No, enjoy ambiguity" }], deadline: new Date(Date.now() + parseTimeRemaining("12 hours")).toISOString(), createdAt: generateCreatedAt("12 hours"), isSpicy: true },
  { id: '2opt_thirst_trap_respect', creator: getRandomUser(), question: "Post thirst trap, can I still demand respect?", options: [{ id: '2opt_thirst_a', text: "Yes, respect unconditional" }, { id: '2opt_thirst_b', text: "It's complicated..." }], deadline: new Date(Date.now() + parseTimeRemaining("4 hours")).toISOString(), createdAt: generateCreatedAt("4 hours"), isSpicy: true },
  ...
  { id: '2opt_charger_return_keep', creator: getRandomUser(), question: "Friend left charger. Return or fair game?", imageUrls: [`https://placehold.co/600x400.png?text=${urlSafeText("ChargerDilemma")}`], options: [{ id: '2opt_charger_a', text: "Return, good karma" }, { id: '2opt_charger_b', text: "Finders keepers!" }], deadline: new Date(Date.now() + parseTimeRemaining("1 day")).toISOString(), createdAt: generateCreatedAt("1 day") },
  ...
  { id: '2opt_podcast_trauma_dump', creator: getRandomUser(), question: "Start podcast to trauma dump or get therapist?", imageUrls: [`https://placehold.co/600x400.png?text=${urlSafeText("PodcastTherapy")}`], options: [{ id: '2opt_podcast_a', text: "Podcast, share story!" }, { id: '2opt_podcast_b', text: "Therapist, help first" }], deadline: new Date(Date.now() + parseTimeRemaining("9 hours")).toISOString(), createdAt: generateCreatedAt("9 hours"), isSpicy: true },
  ...
  { id: '2opt_hoodie_hostage_return', creator: getRandomUser(), question: "Have 3 ex's hoodies. Return or emotional distress compensation?", options: [{ id: '2opt_hostage_a', text: "Return, bigger person" }, { id: '2opt_hostage_b', text: "Keep 'em, trophies" }], deadline: new Date(Date.now() + parseTimeRemaining("4 days")).toISOString(), createdAt: generateCreatedAt("4 days"), isSpicy: true },
  ...
  { id: '2opt_cancel_plans_ugly_day', creator: mockUsers.find(u => u.name === 'Ugly Day Canceller') || getRandomUser(), question: "Woke up feeling ugly. Valid reason to cancel plans?", options: [{ id: 'opt_cpud1_a', text: "Absolutely, self-care!" }, { id: 'opt_cpud1_b', text: "No, push through it" }], deadline: new Date(Date.now() + parseTimeRemaining("45 minutes")).toISOString(), createdAt: generateCreatedAt("45 minutes") },
];
...
const threeOptionPolls: PollSkeleton[] = [
  ...
  { id: '3opt_you_up_reply_chaos', creator: getRandomUser(), question: "Got 'you up?' text at 2 AM. Max chaos/comedy reply?", imageUrls: [`https://placehold.co/600x400.png?text=${urlSafeText("2AM Text")}`], options: [{ id: 'yur1_opt_a', text: "'New phone, who dis?'" }, { id: 'yur1_opt_b', text: "'Yes, and I was just thinking of you... not.'" }, { id: 'yur1_opt_c', text: "Send Wikipedia link to 'Sleep'" }], deadline: new Date(Date.now() + parseTimeRemaining("5 minutes")).toISOString(), createdAt: generateCreatedAt("5 minutes"), isSpicy: true },
...
// Function to shuffle the mockPolls array - call this if you want to reshuffle during runtime for testing
