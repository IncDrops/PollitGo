...
export default async function UserProfilePage({ params }: { params: { userId: string } }) {
  ...
  // Minimal rendering for diagnostics
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 data-testid="user-name-profile">User Profile for: {user.name}</h1>
      <p>User ID: {user.id}</p>
      <p>Username: {user.username}</p>
      <p>If you see this, basic user data ({user.name}) loading and rendering is working.</p>
      <p>Number of polls for this user (not rendered): {polls.length}</p>
